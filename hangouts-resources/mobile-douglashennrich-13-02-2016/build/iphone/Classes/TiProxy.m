...
//Rare exceptions to indicate a bug in the _brincando_1 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_brincando_1 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_brincando_1]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.brincando_1.module"
